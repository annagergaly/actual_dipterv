...
class GCNModel(nn.Module):
    def __init__(self, num_nodes):
        ...
        self.conv1 = torch_geometric.nn.GCNConv(num_nodes, 64)
...
def main():
    ...
    dataset = pytorch_dataset.AutismDataset(train_sites, node_features='correlation', connectome_type='correlation', top_edges=wandb.config.top_edges, brain_atlas=wandb.config.brain_atlas).to(device)
    if wandb.config.brain_atlas == "cc400":
        num_nodes = 392
    else:
        num_nodes = 111
    train_dataset, val_dataset = torch.utils.data.random_split(dataset, [0.8, 0.2])
    test_dataset = pytorch_dataset.AutismDataset([wandb.config.leave_out], node_features='correlation', connectome_type='correlation', top_edges=wandb.config.top_edges, brain_atlas=wandb.config.brain_atlas)
